import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;

import '../../../Base/BaseRequest.dart';
import '../../../Util/ApiManagement/Api.dart';
import '../Model/Dto/ItemCategory.dart';
class ItemCategoryRequest extends BaseRequest {
  Future<void> getList({required Function responseCallback, required BuildContext context}) async {
    const Api api = Api.ItemCategoryGetList;
    final String parameter = api.getParameter();

    http.Response response = await sendRequest(api: api, parameter: parameter, body: "", context: context);

    List<ItemCategoryDto> list =
        responseParsing.getValueList(responseBody: response.body).map((v) => ItemCategoryDto.fromJson(v)).toList();
    responseCallback(list);
  }
}
